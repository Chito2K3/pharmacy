...
foreach ($i in $json) {
    ...
    $epa = 0
    if ($null -ne $i.ending_with_epa_qty) { $epa = [double]($i.ending_with_epa_qty -replace "[^0-9\.\-]","") }
    elseif ($null -ne $i.ending_inv_with_epa_qty) { $epa = [double]($i.ending_inv_with_epa_qty -replace "[^0-9\.\-]","") }
